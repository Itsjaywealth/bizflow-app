...
export async function createInvoiceViaAi({ business, context, payload }) {
  if (!business?.id) {
    throw new Error('A business workspace is required before AI can create invoices.')
  }

  const client = await upsertAgentClient(business.id, context.clients, payload)
  const invoiceNumber = generateInvoiceNumber(context.invoices)
  const numericAmount = Number(payload.amount || 0)
  const issueDate = new Date().toISOString().slice(0, 10)
  const dueDate = payload.dueDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().slice(0, 10)
  ...
  const invoicePayload = {
    ...
    due_date: dueDate,
    notes: payload.note || 'Draft created by BizFlow AI for review before sending.',
    payment_terms: 'Net 7',
  ...
  if (error) {
    logAiToolError('create-invoice', error, business.id)
    throw error
  }
